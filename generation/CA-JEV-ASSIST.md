...
同一事件在结构化 `facts` 未变、且各篇摘要经规范化后均已落在这些 `facts` 中时复用上次结果：仅事实顺序变化、重复相同事实、或纯摘要改写（句子已在 `facts` 里）→ 复用。合并多篇同事件报道并入结构化 `facts`，并保留尚未被 `facts` 覆盖的摘要证据；不把未核实摘要写成事实，也不按输入顺序裁定对错。版本号 / 小数点不当成句号切开。完整 `facts` 由 generate CA 维护。若无法确认摘要已被 `facts` 覆盖，不得复用旧分：在当日额度内重新评估，或跳过 Jev 走原选稿。代码只对结构化 `facts` 做规范化 / 排序 / 去重，不用删词、词数或新词比例来猜测“有没有新事实”。
...
完整 facts 由本次 generate 维护。摘要中的新信息或更正必须先写入 facts；facts 未覆盖的摘要不得复用旧分。
