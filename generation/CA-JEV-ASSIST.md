...
同一事件在结构化 `facts` 未变、且摘要经规范化后已落在这些 `facts` 中时复用上次结果：仅事实顺序变化、重复相同事实、或纯摘要改写（句子已在 `facts` 里）→ 复用。合并多篇同事件报道只并入结构化 `facts`，不会把另一篇摘要追加成新事实。完整 `facts` 由 generate CA 维护；摘要里的新信息或更正必须先写入 `facts`。若无法确认摘要已被 `facts` 覆盖，不得复用旧分：在当日额度内重新评估，或跳过 Jev 走原选稿。代码只对结构化 `facts` 做规范化 / 排序 / 去重，不用删词、词数或新词比例来猜测“有没有新事实”。
